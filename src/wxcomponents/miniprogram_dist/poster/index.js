Component({
    ...
    methods: {
        ...
        onCreate(reset = false,that) {
            !this.data.hideLoading && that.$Toast({ content: '海报生成中', type: 'loading', duration: 0 });
            return this.downloadResource(typeof reset === 'boolean' && reset).then(() => {
                !this.data.hideLoading && that.$Toast.hide();
                const poster = this.selectComponent('#poster');
                poster.create(this.data.config);
            })
            .catch((err) => {
                !this.data.hideLoading && that.$Toast.hide();
                that.$Toast({ content: err.errMsg || '生成失败', type: 'error'});
                console.error(err);
